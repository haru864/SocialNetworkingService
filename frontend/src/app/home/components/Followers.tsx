...
async function getFollowerTweetIds(page: number): Promise<number[]> {
    try {
        const response = await fetch(`${process.env.API_DOMAIN}/api/tweets?type=follower&page=${page}&limit=20`, {
            method: 'GET',
            credentials: 'include'
        });
        if (!response.ok) {
...
const Followers = () => {
    ...
    const loadMoreTweets = async () => {
        const currentTweetIds = await getFollowerTweetIds(page);
